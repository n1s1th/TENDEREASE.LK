...
export default function DocumentUpload({
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      // Store the actual File object so it can be appended to FormData on submit
      setValue('businessRegistrationDocument', selectedFile, { shouldValidate: true });
    }
  };
  ...
  const handleOtherFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
      ...
      const combined = [...otherFiles, ...newFiles];
      setOtherFiles(combined);
      // Store actual File objects array
      setValue('otherDocuments', combined, { shouldValidate: true });
    }
  };

  const removeFile = (fileName: string) => {
    const updated = otherFiles.filter(f => f.name !== fileName);
    setOtherFiles(updated);
