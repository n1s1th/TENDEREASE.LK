...
const mockData = [
  { id: 'T-1001', title: 'Supply of Office Equipment', vendor: 'ABC Pvt Ltd', value: 'LKR 4,850,000', date: '2026-02-01' },
  { id: 'T-1002', title: 'Construction of Community Hall', vendor: 'BuildPro Ltd', value: 'LKR 12,500,000', date: '2026-01-28' },
  { id: 'T-1003', title: 'IT Infrastructure Upgrade', vendor: 'TechSolutions Inc', value: 'LKR 8,200,000', date: '2026-01-25' },
];
...
export default function RecentAwardsPage() {
  ...
  const filteredData = mockData.filter(tender =>
    ...
    tender.vendor.toLowerCase().includes(searchQuery.toLowerCase())
  );
        ...
        <Table>
          <TableHeader>
            <TableRow className="bg-surface-2 border-b-2 border-border">
              ...
              <TableHead className="text-[11px] font-bold uppercase tracking-wider text-text-muted">Awarded Vendor</TableHead>
              <TableHead className="text-[11px] font-bold uppercase tracking-wider text-text-muted">Award Value</TableHead>
              <TableHead className="text-[11px] font-bold uppercase tracking-wider text-text-muted">Award Date</TableHead>
              <TableHead className="text-[11px] font-bold uppercase tracking-wider text-text-muted">Status</TableHead>
            ...
            {paginatedData.length === 0 ? (
              ...
              paginatedData.map((tender) => (
                <TableRow key={tender.id} className="hover:bg-[#faf8f6]">
                  ...
                  <TableCell className="text-text-secondary">{tender.vendor}</TableCell>
                  <TableCell className="text-text-primary font-semibold">{tender.value}</TableCell>
                  <TableCell className="text-text-secondary">{tender.date}</TableCell>
                  <TableCell>
                    <Badge className="bg-success-bg text-success px-2.5 py-1 font-semibold text-[12px] rounded-full">
                      Completed
                    </Badge>
                  </TableCell>
                  <TableCell>
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end" className="bg-surface border-border w-[180px]">
                        ...
                        <DropdownMenuItem className="text-text-primary focus:bg-bg focus:text-text-primary">Download Award Letter</DropdownMenuItem>
                      </DropdownMenuContent>
                    </DropdownMenu>
                  </TableCell>
                </TableRow>
