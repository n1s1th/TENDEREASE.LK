...
export default function SchedulePanel({ scheduledTime }: SchedulePanelProps) {
  ...
  const timeStr = mounted && date ? `${String(date.getHours()).padStart(2, '0')}:${String(date.getMinutes()).padStart(2, '0')}:00` : "--:--:--";
  const dateStr = mounted && date ? date.toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }) : "-- --- ----";
