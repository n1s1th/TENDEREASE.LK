...
export default function OpeningActionPanel() {
  ...
  const canOpen = attendance.length >= 1 && (!session || session.status === 'SCHEDULED' || session.status === 'PENDING_OPENING');
    ...
    <div className="bg-white rounded-3xl border border-gray-100 p-4 shadow-sm flex flex-col h-full relative overflow-hidden">
      ...
      <div className="flex-1 flex flex-col items-center justify-center text-center px-2">
        ...
        <button
          ...
          className={`w-full max-w-[280px] py-4 rounded-[20px] flex flex-col items-center justify-center gap-2 group transition-all duration-300 border ${session?.status === 'OPEN'
        ...
        >
          <div className={`w-8 h-8 rounded-full flex items-center justify-center transition-colors ${session?.status === 'OPEN' ? 'bg-white/20' : !canOpen ? 'bg-gray-200' : 'bg-white/10 group-hover:bg-white/20'
            }`}>
